...
class TestSelfRAGWorkflow:
    ...
    async def test_workflow_initializes(self, mock_llm, mock_vector_store):
        from backend.adaptive_rag.langgraph_workflows.self_rag_workflow import (
            SelfRAGWorkflow,
        )
    ...
    @pytest.mark.asyncio
    async def test_reflect_rewrites_query_with_feedback(
        ...
        mock_rewriter.rewrite = AsyncMock(return_value="What is Python programming?")
